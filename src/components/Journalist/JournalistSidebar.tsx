...
interface JournalistSidebarProps {
  journalist: {
    name: string;
    outlets?: Array<{
      name: string;
...
export default function JournalistSidebar({ journalist }: JournalistSidebarProps) {
  const weeklyPopular = [
    {
      id: '1',
      ...
      {journalist.outlets?.[0] && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center gap-4">
            <img
              src={journalist.outlets[0].logo}
              alt={journalist.outlets[0].name}
              className="w-8 h-8 rounded-full"
            />
            <p className="text-sm text-gray-500 flex-1">Want to see more from <span className="font-bold">{journalist.outlets[0].name}</span>?</p>
